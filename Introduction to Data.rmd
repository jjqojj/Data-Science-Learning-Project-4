...
The affirmations false is No flight is delayed more than 2 hours, because exist a flight with an arrival delayed from roughly 200 minutes. 
...
sfo_feb_flights %>% summarise(sfo_mean=mean(arr_delay),sfo_median=median(arr_delay),sfo_IQR=IQR(arr_delay),n=n())
```
...
The answer is UA (United) and DL (Delta)
...
nycflights_2 <- nycflights %>%  
  filter(dep_type == "on time", arr_type=="delayed")

nycflights_3 <- nycflights %>%  
